...
echo "--------------- Try Creating Task WITHOUT Token (should fail)"
curl -s -X POST $API/api/tasks \
  -H "Content-Type: application/json" \
  -d '{"title": "Test task"}' | jq
echo ""

echo "--------------- Create Task WITH Token (should succeed)"
curl -s -X POST $API/api/tasks \
  -H "Content-Type: application/json" \
...
echo "--------------- List Tasks with New Token"
curl -s -X GET $API/api/tasks \
  -H "Authorization: Bearer $NEW_TOKEN" | jq
echo ""

echo "--------------- Try Login with Wrong Password (should fail)"
curl -s -X POST $API/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "alice@example.com",
...
echo "--------------- Try Invalid Token (should fail)"
curl -s -X GET $API/api/tasks \
